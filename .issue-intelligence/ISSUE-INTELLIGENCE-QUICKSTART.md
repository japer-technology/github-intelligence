# .issue-intelligence 🦞 Quick Start
...
Copy the `.issue-intelligence` folder into your repository root, then run the install script:
...
## What's in the `.issue-intelligence` folder
...
`.pi` supports a wide range of LLM providers out of the box. Set `defaultProvider` and `defaultModel` in `.issue-intelligence/.pi/settings.json` and add the matching API key to your workflow:
