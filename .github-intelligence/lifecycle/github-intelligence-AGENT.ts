 ...
 *   .github-intelligence/state/issues/<number>.json   — maps issue # → session file path
 *   .github-intelligence/state/sessions/<timestamp>.jsonl — the `pi` session transcript
 *
...
// `import.meta.dir` resolves to `.github-intelligence/lifecycle/`; stepping up one level
// gives us the `.github-intelligence/` directory which contains `state/` and `node_modules/`.
...
const sessionsDirRelative = ".github-intelligence/state/sessions";
...
try {
  ...
  await run(["git", "config", "user.name", "github-intelligence[bot]"]);
  await run(["git", "config", "user.email", "github-intelligence[bot]@users.noreply.github.com"]);
  ...
  if (exitCode !== 0) {
    ...
    await run(["git", "commit", "-m", `github-intelligence: work on issue #${issueNumber}`]);
  }
