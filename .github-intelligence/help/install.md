...
This installs the `pi` coding agent and other runtime dependencies defined in `.github-intelligence/package.json`.
...
Set `defaultProvider` and `defaultModel` in `.github-intelligence/.pi/settings.json`:
...
4. Session files appear in `.github-intelligence/state/`
