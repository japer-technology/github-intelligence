# 🗑️ Uninstall Issue Intelligence
...
To completely remove Issue Intelligence, delete all of the following and commit the changes:

### 1. Remove the Issue Intelligence folder
...
```bash
git add -A
git commit -m "Remove issue-intelligence"
git push
```
...
- **No cleanup jobs needed** — Issue Intelligence has no background processes, databases, or external services to shut down. Removing the files is sufficient.

## See Also

- [⏸️ Disable](disable.md) — temporarily stop the agent without removing anything
- [♻️ Reinstall](reinstall.md) — reset or upgrade Issue Intelligence
